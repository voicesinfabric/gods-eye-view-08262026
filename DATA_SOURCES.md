...
| **U.S. DOT & federal live-camera pack** (bundled `config/cctv_sources.us-dot-live.json`) | 134 cameras — 91 direct HLS streams (DelDOT, MoDOT, WisDOT, Nevada DOT, Louisiana DOTD) plus TripCheck stills and 42 page-linked cameras (Maryland CHART, 511GA, AZ511, Alaska 511, DriveNC, and 22 NPS park webcams: Yellowstone, Glacier, Mount Rainier, Yosemite, Great Smokies, Grand Canyon) — with ACCESS LIVE FEED to each operator's own system | Public-courtesy traveler information / NPS federal content; per-entry provider + attribution note in the pack (ODOT permits public use with credit; Maryland requires acknowledging the State; Yosemite imagery is partner-credited, not presumed public domain) | Panel shows the operator per camera; "State DOT traffic cameras" + NPS credit lines |
