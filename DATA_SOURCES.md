...
| **U.S. DOT live-camera pack** (bundled `config/cctv_sources.us-dot-live.json`) | 113 state-DOT/federal cameras — 91 direct HLS streams (DelDOT, MoDOT, WisDOT, Nevada DOT, Louisiana DOTD) plus TripCheck stills and 21 page-linked cameras (Maryland CHART, 511GA, AZ511, Alaska 511, DriveNC, NPS) — with ACCESS LIVE FEED to each operator's own system | Public-courtesy traveler information; per-entry provider + attribution note in the pack (ODOT TripCheck permits public use with credit; Maryland open data requires acknowledging the State of Maryland) | Panel shows the operator per camera; "State DOT traffic cameras" credit line |
