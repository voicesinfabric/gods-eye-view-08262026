...
| **U.S. DOT live-camera pack** (bundled `config/cctv_sources.us-dot-live.json`) | 100 state-DOT/federal cameras — 91 direct HLS streams (DelDOT, MoDOT, WisDOT, Nevada DOT, Louisiana DOTD) plus TripCheck stills and 511GA/AZ511/Alaska/DriveNC/NPS page-linked cameras — with ACCESS LIVE FEED to each operator's own system | Public-courtesy traveler information; per-entry provider + attribution note in the pack (ODOT TripCheck explicitly permits public use with credit) | Panel shows the operator per camera; "State DOT traffic cameras" credit line |
