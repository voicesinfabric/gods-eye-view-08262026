...
| **U.S. DOT & federal live-camera pack** (bundled `config/cctv_sources.us-dot-live.json`) | 146 cameras — 91 direct HLS streams (DelDOT, MoDOT, WisDOT, Nevada DOT, Louisiana DOTD), 11 direct still images (ODOT TripCheck + 10 NPS webcams: Rocky Mountain, Olympic, Crater Lake, Big Bend, Grand Canyon, Yellowstone), and 44 page-linked cameras (Maryland CHART, 511GA, AZ511, Alaska 511, DriveNC, and NPS park webcams across 8 parks) — with ACCESS LIVE FEED to each operator's own system | Public-courtesy traveler information / NPS federal content; per-entry provider + attribution note in the pack (ODOT permits public use with credit; Maryland requires acknowledging the State; Yosemite imagery is partner-credited, not presumed public domain) | Panel shows the operator per camera; "State DOT traffic cameras" + NPS credit lines |
